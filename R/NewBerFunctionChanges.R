...
#TODO
#add functionality so that it takes in path to csv files
#ECber new package name
#add to ccbernew and then add to ecber new package not forked
#add diply and tidyr to converting files, input directory where it looks for all csv files in that
#directory and tries to convert all csv files to excel and if it runs into a problem, 
#continue and add to a log 
#input will be final files sensory signals

#files in folders are the output of people undergoing training for coding
#exported csv files will just contain the final csv files (mayvbe repeated)
#for testing, use sample files 
#underscore t just jmeans that the file has been converted 

#sensory signals+affect, autonomy support+affect are already there

#potential new addition: vocal+affect should be added,just 
#including V instead of TA (tactile/auditory) --> 3 main states only
...
combined_entropy_output <- function(file_name, 
  ...
  fcheck_fail = FALSE
  list_TAV = ber_analyze_file(file_name)
  list_affect = ber_analyze_file_affect(file_name)
  list_affect_autonomy = ber_analyze_file_affect_and_autonomy(file_name)
